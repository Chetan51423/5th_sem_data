#include "isim/work/tb__mux16to1__v/tb__mux16to1__v.h"
#include "isim/work/glbl/glbl.h"
static const char * HSimCopyRightNotice = "Copyright 2004-2005, Xilinx Inc. All rights reserved.";


#include "work/tb__mux16to1__v/tb__mux16to1__v.h"
static HSim__s6* IF0(HSim__s6 *Arch,const char* label,int nGenerics, 
va_list vap)
{
    HSim__s6 *blk = new workMtb__mux16to1__v(label); 
    return blk;
}
...
#include "work/mux16to1/mux16to1.h"
static HSim__s6* IF2(HSim__s6 *Arch,const char* label,int nGenerics, 
va_list vap)
{
    HSim__s6 *blk = new workMmux16to1(label); 
    return blk;
}
...
public:
    ...
    HSimConfigDecl * topModuleInstantiate() {
        ...
        (*cfgvh).addVlogModule("tb_mux16to1_v", (HSimInstFactoryPtr)IF0);
        (*cfgvh).addVlogModule("mux4to1", (HSimInstFactoryPtr)IF1);
        (*cfgvh).addVlogModule("mux16to1", (HSimInstFactoryPtr)IF2);
        (*cfgvh).addVlogModule("glbl", (HSimInstFactoryPtr)IF3);
        HSim__s5 * topvl = 0;
        topvl = new workMtb__mux16to1__v("tb_mux16to1_v");
