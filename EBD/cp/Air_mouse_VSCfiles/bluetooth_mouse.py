...
try:
  ...
  while 1:                                            # While loop to continuesly scan and read data from serial port and execute
      ...
      if data[0] == "L" :                          # Checking if the identifier is "DATAB" which the Arduino sends the values for Left/Right button
            if data[0] == 'L' :                       # If the Left button is pressed
              mouse.press(Button.left)                # The corresponding button is pressed and released
              mouse.release(Button.left)
      if data[0] == 'R' :                       # If the Right button is pressed
            if data[0] == 'R' :                       # If the Left button is pressed
