...
mouse = Controller()
try:
  ser = serial.Serial('COM4',baudrate = 115200)       # Setting Serial port number and baudrate
  while 1:                                            # While loop to continuesly scan and read data from serial port and execute
      ...
      if data[0] == "DATABL" :                          # Checking if the identifier is "DATAB" which the Arduino sends the values for Left/Right button
            if data[0] == 'DATABL' :                       # If the Left button is pressed
              mouse.click(Button.left)                # The corresponding button is pressed and released
              mouse.release(Button.left)
      if data[0] == 'DATABR' :                       # If the Right button is pressed
            if data[0] == 'DATABR' :                       # If the Left button is pressed
              mouse.click(Button.right)                # The corresponding button is pressed and released
